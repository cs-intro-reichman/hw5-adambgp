...
public class Wordle {
    ...
    public static String[] readDictionary(String filename) {
        ...
        In word= new In(filename);
        String []words=new String [count];
        int i=0;
        while (!word.isEmpty()){
            words [i]= word.readString();
            i++;
        }
    ...
    public static String chooseSecretWord(String[] dict) {
        ...
        String chooseSecretWord = dict [random];
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
		for(int y=0;y<secret.length();y++){ 
            ...
            char check = guess.charAt(y);
                
                if(secret.charAt(y)==check){
                    resultRow[y]= 'G';
                }
                else if (containsChar(secret, check)){
                    resultRow[y]= 'Y';}
                    else{
                        resultRow[y]= '_';
                    }
    ...
    public static void main(String[] args) {
        ...
        if (!won) {
          System.out.println("Sorry, you did not guess the word.");  // ... follow the assignment examples for how the printing should look like
